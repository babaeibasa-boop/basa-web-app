...
export function VoucherLayout({ children }: { children: React.ReactNode }) {
  const { user } = useAuth();
  const { theme, toggleTheme } = useTheme();
  const location = useLocation();
  const displayName = user ? `${user.name} ${user.family}`.trim() : null;
      ...
      <header className="sticky top-0 z-40 border-b bg-background/95 backdrop-blur">
        <div className="mx-auto flex h-14 max-w-3xl items-center justify-between px-4">
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-base font-bold">باساچر</h1>
          </div>
          <div className="flex min-w-0 items-center gap-2">
            {displayName && (
              <span className="hidden truncate text-sm text-muted-foreground sm:inline">{displayName}</span>
            )}
            <Button
              variant="ghost"
              size="icon"
